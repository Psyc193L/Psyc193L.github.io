...
Consider some of the main tasks we do in Social Sciences. As in all sciences, we work with data. We **inspect** and **manipulate** that data in different ways, and modify it to make it amenable to **analysis** and **visualizations**. Visualizations and statistical analyses help us explore relationships we have observed in the real world and test for relationships in our experiments/studies. We then use these to **communicate** some findings to the general public.

So why would we want to use R or **any** programming language vs a Graphical User Interface (GUI) software like Excel, JMP, SPSS, or SAS, to do this?

Let's start with some simple, practical, considerations:
...
These are all important, however, one of the main reasons to use a programming language to accomplish our main tasks in Social Sciences is for *Reproducibility*.

Excel and the conventional statistics software like SPSS use a point-and-click GUI, and thus lack a simple way to document and share your analyses. This makes repeating or editing your work later very hard as you'll need to rely on memory and repetition. **It is hard to work backwards from the end result and see how you arrived there because it isn't documented anywhere.** You will have to "reinvent the wheel" to perform the same analysis across different data sets. R is an open-source interpreted *programming language*. Instead of pointing and clicking through some software to accomplish your tasks, you write code that will execute them. All the code you write is read and evaluated line by line in documents you save. This has some very practical consequences for you:

- Selfishly, it allows you to prepare your cleaning, analysis, and visualization code beforehand. As soon as you finish with data collection, you can just run all the code you've written and you'll immediately be done!
- Also, it allows you to automate tasks for yourself. This way, you don't have to repeat the same process of clicking through the GUI and re-doing the same thing many times.
...
* Allows you to create literate programming documents (we will do this in markdown, more on that later) -- where you explain in English what your code is doing. This **drastically** helps you *communicate* your results by making your code be more interpretable, even when you re-visit it weeks/months/years down the line.
  + **"You are always working with at least one collaborator: Future you."<br>-Hadley Wickham**
* Often times you will be working with other non-you collaborators, and this makes doing so easier. People can build off the work (code writing) you've done and see step by step exactly what that work is. 
...
When we talk about R, we are referring to the programming language R itself. We use RStudio to help us write and use R. RStudio is an Integrated Development Environment (IDE). Basically, it provides an interface to make writing and using R code easier.
...
Generally speaking, you'll see 4 sections in your RStudio window. Let's quickly overview each.
...
Additionally, there's two other settings tweaks to make. Go to preferences and then...
...
We will use markdown files because they enable us to more easily annotate our code and create literate programming documents. They also allow us to easily abide by a general programming philosophy called **"separation of concerns,"** which says code should be organized into distinct sections, each handling a specific task. Doing this enables you to quickly navigate your code and know where to look if something isn't working as intended. 

Markdown files can also be used to create documents that help communicate what you are doing in your data analysis pipeline. If you use word, then every time some figure or result changes, you have to go through and copy/paste a bunch of stuff, make a million changes throughout, save new files, it's a nuisance and often error prone. R Markdown files are living, breathing documents. Figures and results are automatically updated if the data is changed. Also, everything is in one place. In fact, what you are looking at here is the output of a markdown file!
...
And adding a set of curly brackets {} with an "r" in it next to the first set of backticks: ```{r}

![](figures/rmd1.png){width=100%}

You can also insert code chunks by pressing control+options+i on OSX or control+alt+i on windows (r is full of nifty shortcuts!)
