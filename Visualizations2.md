...
If the **count**'s were on the y-axis before, then you would just want each `..count..` (numerator above) divided by the sum of all the `..count..`s (denominator above). Luckily, we can just give that to R!
...
In the first histogram you'd probably say this is a unimodal distribution with most of the flipper lengths around 190mm (+/- a few). However, when looking at the second histogram, you'd probably say this distribution looks more bimodal, with one cluster around 190mm and another around 210mm. The same data, visualized using the same type of graph, but two different stories based on choices that you made. You have to be very careful to make sure you are telling the data's story, not your story!
...
Another way to do so is by showing the individual histograms for each of the continuous variables. You can do this by using `ggMarginal()` from the `ggExtra` package.
...
<p style="font-size:10pt">Going forward, we will bring back the habit of making predictions for and/or summarizing the visualizations we create. You will first declare a formal prediction by specifying two mutually exclusive alternative patterns that *could* be observed in your visualizations. You will be working with the same idea (hypothesis), so subsequent predictions can just refer to that hypothesis rather than explicitly stating it. After making the visualization, you'll then write a <u>short</u> summary of what the visualization shows.</p>
...
In the [first lesson][Intro to ggplot2] on visualizations, you saw how many different datasets and distributions were consistent with the same summary statistics (the datasauRus dozen and Anscombe's quartet). This was the rationale for why visualizations are so important. However, if you plot nothing but summary statistics, this is not much better than only reporting the summary statistics as numbers! You never know what your summary statistics could be hiding about your data:
...
2. The jitter added to a visualization is random. Every time you make a jitter plot, or even rerun the code for the same jitter plot, the jitter that's added is random and the visualization will look slightly different!
...
Very often in studies with within-subjects designs (where there are multiple measurements from the same subject -- and particularly in developmental/infant studies), you will see figures that look like this:
...
To fix that, we will have to talk about positions...
...
ggplot2 has a number of position/adjustment arguments that can be used in these cases. In fact, we already have touched on one! Jittering is in essence a position adjustment. So while you can create a jitter plot using `geom_jitter()`, you also could accomplish the same thing using `geom_point()` and setting a position argument. e.g.,

`geom_point(position = "jitter")` or `geom_point(position=position_jitter())`

These in effect do the same thing. However, if you want to include any specific arguments (e.g., specify how much vertical or horizontal jitter to have), you can only do so by using the longer `position_X()` format. This helps create reproducible graphs, as the jitter is otherwise random every time you run the code to create the graph! Changing the positional adjustments can drastically alter the visualization you generate.

Here are a few quick examples of a graph that came out looking funky, and how using positional adjustments can fix that.
...
Note that the difference between *dodge* and *dodge2* is that the latter creates a bit of space between the elements. You'll also notice that, compared to *identity*, the scale of the y-axis for both of these graphs changed.
...
Probably the most common use case for this will be with errorbars and CIs. Observe the following. By default, they are often come out looking funky:
...
However, this can be misleading. Recall that the total number of `species` == "Gentoo" penguins is much greater than `species` == "Chinstrap", even though they don't look TOO different here. So while you can compare the proportions easily, those proportions may correspond to drastically different raw numbers.
...
Looks way better when there are more levels to your variables:
...
* Graphs should be EASILY readable visually, first and foremost. This should be the top design philosophy when constructing your graphs. 
...
* Graphs should be purposeful
  + What is the specific relationship or trend in your data that you are trying to communicate?

* Graphs should facilitate quantitatve interpretation and comparison, and allow for inferential statistics by eye.
  + Represent variability (show the full distribution, include errorbars or confidence intervals).
  + Show relationship trends, means, etc.

* Cool =/= best.
  + Just because you **<u>can</u>** make some crazy complex graph that visualizes a lot of different variables, or might even be interactive and show you a lot of information, doesn't mean that's the best thing to do. Just because you CAN do something doesn't always mean you should. Keep things simple and clean, following the conventions for that type of data or relationship.
...
<!-- 1. I have a hypothesis that a pokemon's Attack and Speed could be related. Make a graph that plots these two variables and shows their relationship. -->


<!-- 2. Pick 3 or 4 pokemon types that you're interested in and modify the graph above to show the difference between those types. -->

<!-- 3. Change the color scheme so that the pokemon types have intuitive / appropriate colors (e.g., water = blue, fire = red), and other aesthetics to make it easy to see the differences between the groups.  -->

<!-- 4. Modify the above graph to see whether those trends are different across the different Generations of pokemon.  -->

<!-- 5. I'm curious about how many of each different pokemon type there are. Make a graph that will show this.  -->

<!--     - For an extra challenge, try to reorder the x-axis. *hint*, use `fct_infreq()` -->



<!-- 6. I wonder if there's a difference in the height of pokemon based on what type they are. Make a graph that will show this. **Note: There are many ways you can do this. Also, feel free to take liberties with what data you graph to help with readability of conveying the overall picture.** -->

<!--     - For an extra challenge, try to reorder the x-axis. *hint*, you have to make modifications to the `Type1` variable before starting the ggplot command. Use `mutate()` and `reorder()`.  -->



<!-- 7. What does the ratio of legendary pokemon look like across generations? Make a graph to show the counts and another one for the proportions.  -->

<!-- 8. Take the plot for #7 and show the differences between generation 5 and 6 (only include these two). -->

<!-- 9. Come up with some graphs of your own. Think of a question or thing you're curious about, or something you can graph, and make a few. While doing so, experiment with different aesthetic values, both mapping and arguments. Write out the question in words and then show the graph underneath.  -->
