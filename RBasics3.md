...
Even if for selfish purposes only, and we ignore how this could impact anyone else, help future you. Don't put future you in a position where they are mad at current you!

## RStudio

### Settings

There are a number of settings tweaks that you'll want to make to help force you into some good habits. 
...
These basically just make it so that variables and other saved data floating around in your global environment get preserved and automatically loaded if R closes. This can create an over-reliance on things that just exist locally in your files. This is very much contra to our whole aim and benefit of reproducibility by using R! You should always be able to easily re-run your code and get anything you need. 

It can also cause weird behavior in between your R sessions because some things may still be saved in your workspace and you don't realize it.

It is thus best practice to not feel attached at all to the stuff in your workspace. If you need something, rerun your code to get it! If you have things that you think would be hard to replicate, that's a problem you need to fix!

Particularly because most of the time you can't predict when things will go wrong...
...
* "Soft-wrap R source files" is *selected*
  + This will just make some of your written code easier to read, so you won't have to scroll horizontally. This only impacts working in RStudio, not your outputs!
...
Something that I strongly encourage doing is creating a new R Project for every project you work on. There is a really great tutorial on R projects [here](https://alexd106.github.io/intro2R/project_setup.html). Rather than reinvent the wheel, I'll just summarize and highlight a few key pieces of information.
...
A file's path specifies where that file is located. It is like a map for your computer, giving it instructions on where to go to look for that specific file. When you download a file (let's call it **"dataset.csv"**), it likely would appear in your downloads folder. Thus, the full file specification would be:

For mac: `/users/userName/Downloads/dataset.csv`

For windows: `C:\Users\userName\Downloads\dataset.csv`

Replacing "userName" with whatever the user name on your machine is.

A file path is constructed by 2 parts: 

1. the file location: `/users/userName/Downloads/` or `C:\Users\userName\Downloads\`
2. the file name: `dataset.csv`

You don't have to specify the full file path if you change R's current *working directory*. Setting your working directory is basically telling R, "I'm going to be working in this specific place. So if you need to look for any files, they will be **here**. Also, if you create any files, create them **here**."

This can cause some issues though. If a file path tells your computer where to look for a file, and I set it to a folder in a specific place on my computer, then that code wouldn't work if you tried to run it. Your computer couldn't access mine, and likely has a totally different file structure!

One of the best features of RStudio Projects is that it helps solve this issue! Instead of using **absolute** file paths like this (where it is fully specified), it let's you use **relative** file paths. In a **relative** file path, all file paths are *relative* to a particular directory. When you use an RStudio Project, upon opening a **.Rproj** file, it will automatically set your working directory to the location of that file! This way, if you want to share your project and analysis files with someone else, you just save and send the entire project directory. When they open it on their computer, the relative path will be appropriately set!
...
You can also easily build a path relative to the top-level directory! Let's say we made a "data" folder in our R Project directory (we'll talk more about directory organization later), and put **"dataset.csv"** in there. We can use `here()` to get full relative file path like so:
...
It's also a good practice to create a specific introduction file in the root of each project. Explain how to navigate the directory, where everything is, a synopsis of what the project/assignment is about, any of the materials needed to run the code you have, etc. Future you will thank you for doing this!
...
Given our goals of reproducibility, one of the best things we can do is make sure our code is easy to understand and replicate. To do this, we will establish the routine of **literate programming**. 
...
When we talked briefly before about how the different types of R files, we introduced markdown files. These are what we will mainly be using. Not only do they allow the combination of R code and text, but they also incorporate markdown syntax (which you can think of as it's own mini language), HTML, and CSS! Thus, markdown files afford a powerful combination of languages to help create literate programming documents with easy to understand and reproduce analyses.

However, the true power comes from the ability to take markdown files and generate many different kinds of documents. When combined with some other packages (e.g., `knitr`), Markdown files can be turned into:
...
In fact, this course website, all the course content, my personal website (and all parts of it), are all created in RStudio and written with a lot of R and Markdown!

#### Syntax

We won't go too in depth about Markdown syntax, that could be a whole course on its own! Here are a couple fundamental things though:

You can insert code chunks with the keyboard shortcut: ctrl+option/alt+i
...
You can also write R code inline, or, without a chunk. You can do this by writing a set of backtics, r (lowercase) after the first, then a space and then your code. For example:
...
There are many best practices that are good to incorporate in your coding. I'm going to highlight some of the main ones here, but include a list of many others at the bottom (that for time purposes we won't cover extensively).
...
Object names must start with a letter, and can only contain letters, numbers, `_` and `.`. File names require the same, but can also use `-`. However, you should avoid using `.` for both, and you should **never** use spaces! You want your names to be descriptive, so you'll need a convention for multiple words. 

![](figures/coding_cases.png){width=100%}
<p style="font-size:6pt">Artwork by @allison_horst</p>

I use camelCase, where the first word is not capitalized and the first letter of each subsequent word is. It is technically the most efficient in terms of keystrokes, but you can run into some issues when using acronyms or singular letters (e.g., "RStudio technically breaks this rule). I have seen more people recommending snake_case and arguing against the use of capital letters. So I would generally only recommend camelCase or snake_case.
...
Avoid writing code that takes up a lot of space horizontally. Use strategically placed line breaks and indentations, particularly after each argument/chunk of your code. You can use the keyboard shortcut ctrl+i to get R to automatically indent appropriately for you line-by-line!
...
* Index and columns and subset rows by names or filtering, not numbers. Their order/position may change. Their name likely will not. 
...
* Use version control. Version control are systems that save changes as separate files. Using a system like git is helpful, and prevents you from having tons and tons of older versions of files on your computer. We won't cover github in class (though maybe we will), but there are many resources. 

As far as your markdown files themselves...

* All the needed packages are loaded at the top of your document
* No package installations called/executed in your Rmd
* No warnings/messages printed when you load packages (turn on the options to hide these)
* Use of markdown formatting to make your doc easier to follow
* Relative file paths only using the `here` package
* No chunks with errors!
* **NEVER EVER** printing out an entire dataset to screen (> 10 rows). This is never necessary.
